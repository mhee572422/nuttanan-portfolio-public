const fs = require('fs');
let content = fs.readFileSync('src/components/Navbar.tsx', 'utf8');

if (!content.includes('useAuth')) {
  content = content.replace(
    "import { useLanguage } from '../context/LanguageContext';",
    "import { useLanguage } from '../context/LanguageContext';\nimport { useAuth } from '../context/AuthContext';"
  );
  
  content = content.replace(
    "export default function Navbar() {",
    "export default function Navbar() {\n  const { user, signInWithGoogle, logout } = useAuth();"
  );
  
  // Add auth button next to language toggle
  const authButton = `            {user ? (
              <div className="hidden md:flex items-center gap-4 border-l border-white/10 pl-6 ml-2">
                <div className="flex items-center gap-2">
                  <img src={user.photoURL || ''} alt="Profile" className="w-6 h-6 rounded-full border border-[#C5A059]" />
                  <span className="text-[10px] text-white/80 uppercase tracking-widest truncate max-w-[100px]">{user.displayName?.split(' ')[0]}</span>
                </div>
                <button 
                  onClick={logout}
                  className="text-[10px] text-white/40 hover:text-white uppercase tracking-widest transition-colors"
                >
                  {language === 'en' ? 'Logout' : 'ออกจากระบบ'}
                </button>
              </div>
            ) : (
              <div className="hidden md:flex items-center border-l border-white/10 pl-6 ml-2">
                <button 
                  onClick={signInWithGoogle}
                  className="text-[10px] text-[#C5A059] border border-[#C5A059]/30 hover:bg-[#C5A059]/10 px-3 py-1.5 rounded-sm uppercase tracking-widest transition-colors flex items-center gap-2"
                >
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
                  {language === 'en' ? 'Sign In' : 'เข้าสู่ระบบ'}
                </button>
              </div>
            )}`;
            
  const targetRegex = /<div className="hidden md:flex items-center gap-6">\s*\{navLinks\.map\(\(link\) => \([\s\S]*?<\/div>/;
  const match = content.match(targetRegex);
  
  if (match) {
    content = content.replace(targetRegex, match[0] + '\n' + authButton);
  }
  
  fs.writeFileSync('src/components/Navbar.tsx', content, 'utf8');
}
